...
app.put("/settings", async (req, res) => {
	try {
		...
		connection.execute(sqlSelect, [userID], async (error, results) => {
			...
			if (tel !== undefined) {
				const sqlUpdateTel =
					"UPDATE users SET tel = ? WHERE userID = ?";
...
app.get("/userList", async (req, res) => {
	try {
		...
		let sql =
			"SELECT post.*, users.*, COUNT(seat.userID_rider) AS join_user " +
			"FROM post " +
			"JOIN users ON post.userID = users.userID " +
			"LEFT JOIN seat ON seat.userID_rider = users.userID ";
...
app.get("/postDetail/:postID", async (req, res) => {
	...
	let sql = `SELECT post.*, users.*, COUNT(seat.userID_rider) AS join_user
  FROM post 
  JOIN users ON post.userID = users.userID
  LEFT JOIN seat ON seat.userID_rider = users.userID `;
